...
6. **Evaluate-* rubrics have diverged max-score values — Keep, but note.** `evaluate-skill` is 120 pts (8 dimensions), `evaluate-agent` / `evaluate-hook` / `evaluate-docs` are 115 pts (5-6 dimensions). `evaluate-batch` handles this correctly via `percentage` normalization (line 207: *"The `percentage` field normalizes across types — use it for cross-type comparisons and thresholds (e.g., 85% quality gate)"*). The divergence is principled (skills have an extra D8 Practical Usability dimension agents don't need), not drift. Worth noting because any future rubric addition on one evaluator but not the others will widen the gap. Not v3-blocking.
...
**User resolutions surfaced during review:** (pending — filled in during/after user feedback).
...
8. **Rubric divergence: evaluate-skill is 120 pts (8 dims) while evaluate-agent/hook/docs are 115 pts (5-6 dims).** `evaluate-batch` normalizes via `percentage` (line 207), so cross-type comparisons work. Divergence is principled. Worth noting for future rubric additions — adding a dimension to one evaluator but not others would widen the gap.
