...
test_session_start_source_capture() {
    ...
    local hook="session-start.sh"
    local source_val
    for source_val in startup resume clear compact; do
        local sid="test-src-${source_val}-$(date +%s%N)"
...
test_call_id_capture() {
    ...
    local sid="test-callid-bash-$(date +%s%N)"
    local tid="toolu_01TESTBASHCALLID${RANDOM}"
    ...
    local sid2="test-callid-sessionstart-$(date +%s%N)"
...
test_grouped_read_guard() {
    report_section "=== grouped-read-guard.sh (dispatcher) ==="
    local hook="grouped-read-guard.sh"
...
if [ -z "$FILTER" ]; then
    ...
    case "$FILTER" in
        ...
        call-id*|callid*|tool-use-id*) test_call_id_capture ;;
        grouped-bash*|bash-dispatcher*) test_grouped_bash_guard ;;
        grouped-read*|read-dispatcher*) test_grouped_read_guard ;;
        grouped*|dispatcher*) test_grouped_bash_guard; test_grouped_read_guard ;;
