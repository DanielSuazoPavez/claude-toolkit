...
- **The standalone-vs-dispatched dual-mode hooks** (block-config-edits, git-safety, secrets-guard) export `match_<name>` and `check_<name>` functions for the dispatcher to source. Code-shape implications are a `clarity.md` call — performance is unaffected because the dispatcher just sources the file (parse cost paid once per dispatcher invocation). **Caveat (added post-secrets-guard alignment, May 2026):** "exposes match/check functions" is necessary but not sufficient for "all `main()` paths route through them." Pre-alignment, `secrets-guard.sh`'s standalone `main()` Read handler bypassed `match_secrets_guard_read`/`check_secrets_guard_read` (inlined `check_path` + the `.git/config` test) and the Grep pair was orphaned (no caller at all — only the dispatched Bash branch used its pair). Branch `refactor/secrets-guard-align-branches` routed all three branches through their pairs and moved the `~/.ssh/config` allowlist into `detection-registry.json` (closing a superset-invariant violation found in the process). `block-config-edits` and `git-safety` retain the same partial pattern: a pair for the Bash branch only, with the other tool branches (Write/Edit; EnterPlanMode) inlined in `main()`. Tracked as P1 follow-ups (`hook-audit-01-block-config-edits-write-edit-pair`, `hook-audit-01-git-safety-enterplanmode-pair`).
