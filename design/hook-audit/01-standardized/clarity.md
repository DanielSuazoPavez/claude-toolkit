...
**Follow-up (May 2026, post-`refactor/secrets-guard-align-branches`):** while planning the Shape A test layer (`hook-audit-01-shape-a-match-check-pairs`), reading the file in detail surfaced an inline-vs-functions divergence the audit missed: standalone `main()`'s Read handler did not call `match_secrets_guard_read`/`check_secrets_guard_read` (inlined the path check), and the Grep pair was orphaned (no caller at all). The Read-pair bypass also masked a real superset-invariant violation — the predicate didn't admit `~/.ssh/config`, but `_match_path_registry` had an explicit fallback that did, so `check_*` would have acted on input the predicate excluded. Resolution: routed both `main()` branches through their pairs, deleted the now-unused `check_path` wrapper, and added an `ssh-config` entry to `detection-registry.json` (anchored pattern `\.ssh/config$` to preserve exact-match behavior). Recommendation here unchanged — keep the file as one — but the inline-vs-functions consistency is a separate concern from the per-file LoC framing this proposal addressed.
