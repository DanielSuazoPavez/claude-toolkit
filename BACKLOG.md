...
- **[TOOLKIT]** Standardize saving route of generated documents (`standardize-generated-docs`)
    - **status**: `idea`
    - **scope**: `agents, skills`
    - **notes**: Standardize output paths and file naming for documents generated by agents and skills. Pattern: `{timestamp}-{agent/skill}-{context}.md`. Define convention for save location (e.g. `.claude/reviews/`).

- **[TOOLKIT]** Create resource dependency verification script (`script-verify-deps`)
    - **status**: `idea`
    - **scope**: `toolkit, tests`
    - **notes**: Verify that resources with dependencies on other resources are properly set up — hooks referencing valid paths, skills referencing existing agents, etc.
