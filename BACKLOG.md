...
- **[SCRIPTS]** Exclude `validate-dist-manifests.sh` from consumer syncs (`validate-all-fails-in-consumers`)
    - **scope**: `scripts`
    - **notes**: Reported by claude-sessions via suggestions-box (`suggestions-box/claude-sessions/20260425_114037_issue.txt`, 2026-04-25). `validate-all.sh` runs `validate-dist-manifests.sh` as part of the suite, but that validator is workshop-internal — it expects `dist/raiz/MANIFEST` and `dist/base/EXCLUDE`, which only exist in the toolkit repo source tree. In a consumer project, `bash .claude/scripts/validate-all.sh` exits non-zero with `Missing: dist/raiz/MANIFEST` / `Missing: dist/base/EXCLUDE` even when the consumer is otherwise healthy (observed during `/setup-toolkit` Phase 3 after a clean sync to 2.64.1: 8 setup checks passed, validate-all.sh failed solely due to this). Fix options: (a) exclude `validate-dist-manifests.sh` from sync (workshop-internal per the 2.63.15 SCRIPTS.md reframing — consistent with how other workshop-only scripts are gated); (b) early-exit 0 with an info message when `dist/` is absent in cwd. Lean: (a) — the validator has zero purpose in a consumer project; gating belongs in `dist/base/EXCLUDE` (and verify it's already absent from `dist/raiz/MANIFEST`).
    - **depends on**: none
