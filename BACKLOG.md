...
- **[TOOLKIT]** Sync settings.json template to current format (`settings-template-update`)
    - **status**: `idea`
    - **scope**: `toolkit`
    - **notes**: The settings.json template used by install.sh may drift from the actual format. Add a check or sync mechanism to keep them aligned.
