...
- **[TOOLKIT]** Raiz toolkit — scoped subset for onboarding coworkers (`toolkit-raiz-pack`)
    - **status**: `in-progress`
    - **scope**: `toolkit`
    - **branch**: `feat/raiz-toolkit`
    - **notes**: Curated slice defined by raiz MANIFEST for coworkers getting Claude Code. 6 skills, 5 hooks, 3 agents, 2 memories, 3 templates. No personal preferences. Design doc: `.claude/output/design/starter-pack.md`
    - **related**: `toolkit-raiz-publish`

- **[TOOLKIT]** Auto-publish raiz toolkit to separate repo (`toolkit-raiz-publish`)
    - **status**: `in-progress`
    - **scope**: `toolkit`
    - **branch**: `feat/raiz-toolkit`
    - **depends-on**: `toolkit-raiz-pack`
    - **notes**: GitHub Action on push to main — reads raiz MANIFEST, runs publish.sh (copies resources, trims cross-refs and settings), pushes to `claude-toolkit-raiz` repo. Deploy key for scoped auth. Needs: target repo created, deploy key pair configured.
