...
- **[TOOLKIT]** macOS compatibility for shipped bash scripts (`macos-bash-compatibility`)
    ...
    - **notes**: Parent task — see subtasks `macos-grep-pcre`, `macos-find-printf`, `macos-bash4-policy`, `macos-mktemp-md5sum`, `macos-loud-errors`, `macos-ci-runner`, `macos-readme-platform` (ordered by descending impact-per-effort). Toolkit's bash scripts assume GNU userland + bash 4+; on macOS (BSD userland + `/bin/bash` 3.2) several validators and `setup-toolkit-diagnose.sh` produce silent-correctness bugs because errors are suppressed with `2>/dev/null` and downstream comparisons pass vacuously over empty data. Surfaced 2026-04-27 by the toolkit's biggest external consumer running `setup-toolkit-diagnose.sh` on macOS — visible symptom was `grep: invalid option -- P` noise; underlying issue is that `setup-toolkit-diagnose.sh`, `make validate`, and `make backlog` are all structurally broken on stock macOS. Analysis report (referenced above) catalogs 38 hard-break sites across 13 files in three independent classes: PCRE grep (22 hits), GNU `find -printf` (6 hits), bash 4+ features (8 hits). Success criterion: macOS consumer can run `setup-toolkit-diagnose.sh`, `make check`, and `make backlog` cleanly.

- **[TOOLKIT]** Replace `grep -oP` (PCRE) with portable extraction (`macos-grep-pcre`)
    - **status**: `ready-for-pr`
    - **scope**: `toolkit`
    - **branch**: `fix/macos-grep-pcre`
    - **relates-to**: `macos-bash-compatibility:depends-on`
    - **references**: `output/claude-toolkit/analysis/20260427_1322__analyze-idea__macos-bash-compatibility.md`
    - **notes**: All 22 hits across 6 files migrated. End-to-end output of `verify-resource-deps.sh` is byte-identical to pre-change on real toolkit data; counts unchanged (9/0/17/124/3/28/37). `make check` green on Linux. Two new tests added (`test-validate-settings-template.sh`, `test-validate-safe-commands-sync.sh`) so each migrated script has dedicated coverage. Pre-flight gotcha caught at byte-diff: line 250 of `verify-resource-deps.sh` (``\`name\` agent``) is multi-match-per-line — initial pre-flight using `\b` in awk silently treated it as literal `b` and missed 3 references; upgraded to looping awk with `[^[:alnum:]_]` boundary. **Awaiting**: user opens PR + asks macOS consumer to re-run `setup-toolkit-diagnose.sh` to confirm the visible `grep: invalid option -- P` errors are gone.
