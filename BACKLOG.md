...
- **[TESTS]** Review test suite performance (`tests-perf-review`)
    - **scope**: `tests`
    - **notes**: Surfaced 2026-04-26. `make test` currently runs in ~2 minutes (13/13 files); the dominant cost is `run-hook-tests` (~64s) and pytest (~30s). No formal perf baseline exists. Goal: profile each test file, identify the long tail (likely fixture setup, sqlite3 schema seeding, or sequential I/O that could parallelize), and decide whether to optimize or accept. Specific candidates: (a) hook-test-setup.sh creates a per-process temp dir and seeds DBs — check whether parallel runners contend on shared paths; (b) test-validate-resources-indexed at ~13s seems large for a validator — re-check what it scans; (c) pytest at ~30s for 36 tests is ~830ms/test — investigate slow ones. Decide: optimize, mark slow tests with `@pytest.mark.slow` and split fast/slow targets (per essential-conventions-code_style §4 "Test Scope" guidance: "Defer until make test exceeds ~10s or you notice yourself avoiding it" — we're already at 2 min), or just document the baseline so we know what "normal" looks like. Inputs: `tests/.logs/` per-file logs from `run-all.sh`, `tests/run-hook-tests.sh` parallelism config (HOOK_TEST_JOBS), pytest with `--durations=20`. Output: a perf report doc with per-file timings and recommendations.
    - **depends on**: none

- **[CLI]** Add `claude-toolkit lessons demote` subcommand (`lessons-demote-cli`)
    - **scope**: `cli`
    - **notes**: Surfaced 2026-04-26 during the Key-tier crystallization sweep. CLI exposes `promote`/`deactivate`/`absorb`/`crystallize` but no inverse of `promote`. The Key Promotion Contract in `manage-lessons/SKILL.md` §4.5 lists "demote back to recent" as one of three valid paths when revisiting a Key lesson, but the workaround is currently raw SQL: `sqlite3 ~/.claude/lessons.db "UPDATE lessons SET tier='recent' WHERE id='<ID>';"`. Add a thin subcommand mirroring `promote`'s shape (single `--id`, sets tier='recent', no other side effects). Low priority: the sweep that motivated this didn't actually need demotion (all 7 active Key lessons absorbed into docs/skills cleanly), and SQL works fine for the rare case. Add when convenient. Design context: `output/claude-toolkit/design/20260426_1623__design__key-lessons-review.md`.
    - **depends on**: none
