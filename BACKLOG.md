...
- **[TESTS]** Flaky `test-backlog-query.sh` "filters by scope=skills" under parallel `make check` (`backlog-query-scope-flake`)
    - **scope**: `tests`
    - **notes**: Surfaced 2026-04-27 during `diag-statusline-scope`. The "filters by scope=skills" assertion (`expect_output "filters by scope=skills" "High priority skill" scope skills`) failed once under parallel `make check` but passes 100% in isolation (`bash tests/test-backlog-query.sh -q` → 86/86). Output showed the correct text (`[SKILLS] High priority skill`) yet `grep -qF` reported no match. Same class as the `diag-orphan-flake` — likely a transient WSL2 subprocess hiccup under parallel load, not a test logic defect. **Action if it recurs**: add diagnostic output in the failure branch (pipe length, hex dump) to distinguish corruption from a genuine grep miss.
