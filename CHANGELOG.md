...
### Fixed
- **hooks**: `secrets-guard` now blocks reads of shell and REPL history files. Before this, `cat ~/.bash_history` and `grep AKIA ~/.zsh_history` passed — same threat shape as reading `.env` (history routinely holds pasted tokens, leaked env exports like `AWS_SECRET_ACCESS_KEY=... aws ...`, `curl` with `Authorization` headers, ad-hoc `echo $TOKEN`). One `shell-history` entry in `detection-registry.json` covers 12 basenames in a single `$`-anchored alternation: `.bash_history`, `.zsh_history`, `.sh_history`, `.history`, `.python_history`, `.node_repl_history`, `.psql_history`, `.mysql_history`, `.sqlite_history`, `.rediscli_history`, `.irb_history`, `.lesshst`. The Bash branch gets an explicit `HISTORY_RE` reader rule — without it the matcher fires (path is in the registry alternation) but `check_secrets_guard` returns 0 and the command silently passes. `$`-anchoring means rotated forms (`.bash_history.bak`) don't match; the rule targets the canonical paths.

### Changed
- **tests**: `tests/hooks/test-secrets-guard.sh` adds 14 cases — Read/Bash/Grep blocks across `.bash_history`, `.zsh_history`, `.python_history`, `.psql_history`, plus three allows pinning the boundaries (non-home look-alike under `/project/`, `.bash_logout` rejected by the `$` anchor, `history` builtin not picked up).

### Notes
- Consumed: `.claude/hooks/secrets-guard.sh` and `.claude/hooks/lib/detection-registry.json` ship in base sync (and are in `dist/raiz/MANIFEST`), so the fix reaches all consumers including raiz. Raiz sidecar carries a real entry.
- Source: `suggestions-box/bm-sop/20260505_164818_issue.txt` (filed as backlog `secrets-guard-shell-history-coverage`, P0).
- **backlog**: grounded `eval-claude-mem` analysis into concrete next steps and removed the umbrella eval task. Filed `lessons-redesign` (P0, with calibration note that the analysis miscounted absorbed/crystallized lessons as inactive), `secrets-guard-shell-history-coverage` (P0, from `suggestions-box/bm-sop` — landed in this version), `hook-bypass-adversarial-survey` (P3), `timestamp-standard-define-and-backfill` (P1, project-wide `YYYYmmddTHHMM__<source>__<slug>.md`), and `plan-name-normalization-hook` (P0, depends on the standard — PostToolUse(ExitPlanMode) hook that writes a normalized sibling next to the whimsy-named original).
...
### Changed
- **hooks + cli**: `CLAUDE_ANALYTICS_LESSONS_DB`, `CLAUDE_ANALYTICS_SESSIONS_DB`, and `CLAUDE_ANALYTICS_HOOKS_DB` now default to `$HOME/claude-analytics/<name>.db` instead of `$HOME/.claude/<name>.db`. The four touched call sites — `.claude/hooks/lib/hook-utils.sh:179` (`_resolve_project_id` → sessions.db), `.claude/hooks/surface-lessons.sh:29,85` (lessons.db + hooks.db), `.claude/hooks/session-start.sh:161` (lessons.db), and `cli/lessons/db.py:40-41` (lessons.db + sessions.db) — now point at where the claude-sessions indexer actually writes the DBs. `CLAUDE_ANALYTICS_HOOKS_DIR` was already at `$HOME/claude-analytics/hook-logs` and is unchanged. **Behavioral note for consumers**: anyone relying on the old `$HOME/.claude/*.db` default without setting env vars will now look at the new path; users with the analytics dir already at `~/claude-analytics/` (the documented location) see no change.
- **docs**: `.claude/docs/relevant-toolkit-env_vars.md` defaults table, `.claude/docs/relevant-toolkit-lessons.md` location refs (×2), `.claude/skills/setup-toolkit/SKILL.md` interactive prompts (×4 — fallback note + 3 path prompts), `.claude/skills/manage-lessons/SKILL.md` demote example, `cli/CLAUDE.md` + `cli/lessons/CLAUDE.md` defaults, `docs/getting-started.md` env-var defaults block (×3), `docs/indexes/HOOKS.md` prose (×2). Settings.json `_env_config` `_note` and per-key descriptions for `CLAUDE_ANALYTICS_LESSONS_DB` / `CLAUDE_ANALYTICS_HOOKS_DB` updated to match.
- **tests**: `tests/perf-session-start.sh`, `tests/perf-surface-lessons.sh` LESSONS_DB defaults; comment-only updates in `tests/perf-detection-registry.sh`, `tests/lib/hook-test-setup.sh`, `tests/hooks/test-session-start.sh`.

### Added
- **tests**: `tests/hooks/test-match-check-pairs.sh` — Shape A test layer for the 9 dual-mode hooks (`auto-mode-shared-steps`, `block-config-edits`, `block-credential-exfiltration`, `block-dangerous-commands`, `enforce-make-commands`, `enforce-uv-run`, `git-safety`, `secrets-guard`, `suggest-read-json`). Sources every hook in-process and calls `match_*`/`check_*` directly, asserting on rc + `_BLOCK_REASON`. 76 cases pass + 1 live xfail probe for the still-open interleaved-quote evasion in `match_dangerous` (`'r'm -rf /` shape — see backlog id `hook-audit-01-block-dangerous-interleaved-quote-predicate`). Wall ~410ms standalone, ~2.1s under the parallel runner — well under any Shape B file. Closes `hook-audit-01-shape-a-match-check-pairs`. Predecessor analysis: `design/hook-audit/01-standardized/testability.md`. (Folded forward from `[Unreleased]` — work landed earlier but was deferred from 2.81.7 since it's workshop-internal.)

### Notes
- **Out of scope (intentional)** for the analytics relocation: `CLAUDE_ANALYTICS_USAGE_DB` and `usage-snapshots/` references were left alone — they aren't consumed by toolkit code (they're claude-sessions / statusline-capture surfaces). Generic `~/.claude/hooks-logs/` examples in `create-hook` / `evaluate-hook` SKILLs are example logging paths unrelated to `CLAUDE_ANALYTICS_HOOKS_DIR` and stay as-is.
- **Shape A vs Shape B** (test layer): layer is purely additive. Shape B (subprocess-fork-per-case end-to-end coverage) stays unchanged. Wall budgets in `tests/.logs/test-*.dur` for existing Shape B files are unaffected; `make test` overall wall stays bounded by the slowest Shape B file (~14.7s for `test-auto-mode-shared-steps`).
- **Workshop-internal artifacts**: `tests/hooks/test-match-check-pairs.sh` is not in `dist/raiz/MANIFEST` or `dist/base/EXCLUDE` — no consumer behavior change for that piece. The analytics-relocation parts above *do* reach raiz consumers (see sidecar).

## [2.81.7] - 2026-05-03 - per-hook PERF-BUDGET-MS headers stop V20 false-positives

### Added
- **hooks**: `# CC-HOOK: PERF-BUDGET-MS: scope_miss=N, scope_hit=N` header lines added to all 13 standardized hooks (`approve-safe-commands`, `auto-mode-shared-steps`, `block-config-edits`, `block-credential-exfiltration`, `block-dangerous-commands`, `detect-session-start-truncation`, `enforce-make-commands`, `enforce-uv-run`, `git-safety`, `log-permission-denied`, `log-tool-uses`, `secrets-guard`, `suggest-read-json`). Budgets derived from the V21+ paired probe (`design/hook-audit/01-standardized/performance.md`) — covers worst-observed `duration_ms` over 43 combined samples per fixture (N=20 quiet probe + N=23 from concurrent loaded `make check` validate.sh runs) with ~10% headroom over `max(observed)`. V20 warnings on category 01 drop from 13 to 0 across `make check` runs; the structural floor is now budgeted, not flagged. Closes `hook-audit-01-perf-rerun-paired`.

### Changed
- **tooling**: `design/hook-audit/measurement/probe/run-per-hook-probe.sh` — extended to paired-outcome fixtures (V21+). HOOKS_AND_FIXTURES now carries `(hook, outcome, fixture)` triples; TSV header gains an `outcome` column; per-fixture stats label includes outcome (e.g., `block-dangerous-commands/blocked (smoke)`). Runtime ~5min for 25 triples × 2 modes × N=30 + warmup. Original V18 single-fixture probe outputs (`per-hook-N30.tsv`/`.summary`) kept as historical record; V21+ outputs land in `per-hook-N30-paired.tsv`/`.summary`.
- **docs**: `design/hook-audit/01-standardized/performance.md` — refreshed methodology, per-hook totals split by (block, pass) outcome, check-body cost split by outcome, added direct `duration_ms` companion table (V20 budget basis), rewrote fixture-asymmetry caveat as "coverage notes", added `PERF-BUDGET-MS` recommendation table with derivation and load-noise rationale, updated three "hooks flagged for follow-up" subsections (loggers, big-check-body cluster) with paired numbers. Closed `Open: per-fixture-outcome paired measurement` and removed the matching `Confidence` caveat.
...
### Notes
- Raiz sidecar: `skip: false`. 9 of the 13 standardized hooks ship to raiz consumers via `dist/raiz/MANIFEST` (no `enforce-make-commands`, `enforce-uv-run`, `log-tool-uses`, `detect-session-start-truncation`). The `PERF-BUDGET-MS` header is read by `hook-framework/validate.sh` V20 in raiz consumers' `make check` — they will see the same V20-warning reduction. No runtime behavior change for the hooks themselves; the header is metadata-only.
- Out of scope (filed elsewhere): dispatcher V20 budgets (`grouped-bash-guard`, `grouped-read-guard` — see `02-dispatchers/inventory.md:145`); session-context V20 budgets (`session-start`, `surface-lessons` — see `03-session-context/inventory.md:139`); `detect-session-start-truncation` block-outcome fixture (depends on `hook-audit-01-detect-truncation-injected-fixture` P3); lazy-resolution patch landing.
- Bundle preserves a clean pre-/post-patch comparison: this version captures the structural per-outcome cost; the lazy-resolution patch (when it lands) will close the real-vs-smoke gap measured here.
...
### Fixed
- **hooks**: `secrets-guard.sh` Read branch on `~/.ssh/config` — pre-cleanup, the standalone `main()` Read handler inlined `check_path` and `_match_path_registry` had an explicit fallback for the `$HOME/.ssh/config` exact path, while `match_secrets_guard_read` (which only the dispatched path used) didn't admit the same input. Routing `main()` through the existing match/check pair (see *Changed* below) surfaced this as a real superset-invariant violation: `check_*` would have acted on input the predicate excluded. Resolved by promoting `ssh-config` to a first-class `detection-registry.json` entry (anchored pattern `\.ssh/config$` to preserve exact-match behavior) so both the predicate and the check derive their input from the same registry alternation.

### Changed
- **hooks**: `secrets-guard.sh` standalone `main()` — Read and Grep handlers now route through their existing `match_secrets_guard_read`/`check_secrets_guard_read` and `match_secrets_guard_grep`/`check_secrets_guard_grep` pairs (the dispatched paths already did). Pre-cleanup, the Read pair was bypassed by inline logic and the Grep pair was orphaned (no caller at all — no Grep dispatcher exists, and `main()` ignored the pair). The Bash branch was already aligned. All three branches now follow the same shape; the now-unused `check_path` helper is removed.

### Added
- **hooks**: `lib/detection-registry.json` — new `ssh-config` entry (`kind: path`, `target: stripped`, `pattern: \.ssh/config$`). Replaces the explicit fallback in `_match_path_registry` so the registry is the single source of truth for path-shape detection. 23 entries total (was 22).

### Notes
- 89/89 `tests/hooks/test-secrets-guard.sh` Shape B cases continue to pass; full `make check` green. No behavior change for downstream consumers — the visible effect is that `~/.ssh/config` now blocks consistently across both standalone and dispatched modes (it always blocked in standalone, it always blocked in dispatched; the shapes are now uniform).
- **hook-audit/01 amendments.** `inventory.md`, `testability.md`, and `clarity.md` Proposal 3 each gain a caveat noting that "exposes match/check functions" (the audit's working definition of dual-mode) is necessary but not sufficient for "all `main()` paths route through them." Records that `secrets-guard` is now uniformly aligned and that `block-config-edits` and `git-safety` retain the same partial pattern (Bash-only pair; Write/Edit and EnterPlanMode inlined). Existing recommendations all unchanged.
- **Backlog churn.** Promoted seven `hook-audit-01-*` follow-ups from P1 to P0 to reflect active scope (paired-outcome-fixtures, suggest-read-json-nonexistent, block-dangerous-quote-predicate, test-log-tool-uses, test-detect-session-start-truncation, shape-a-match-check-pairs, superset-invariant-doc-and-test). Filed three new follow-ups uncovered by the alignment work: `hook-audit-01-block-config-edits-write-edit-pair` (P1), `hook-audit-01-git-safety-enterplanmode-pair` (P1), `hook-audit-01-secrets-guard-grep-git-config` (P2). Filed one P99 task (`hook-audit-01-secrets-guard-allowlist-centralize`) for a cosmetic dedup of the `.example`/`.template` allowlist regex.
...
- **hooks**: `auto-mode-shared-steps.sh` simplified to a single regex test against `settings.json` `permissions.ask` — every Bash entry is in scope, no carve-outs. The hook's job: stop the classifier-driven `permission_mode=auto` from auto-approving operations listed in `permissions.ask`. The previous 30-line gh-cascade and the curl/wget-specific filters (Authorization-header detection, `api.github.com` capability check) are gone — they were dead weight once curl/wget block unconditionally. Network egress (`curl`, `wget`) belongs in interactive mode where `permissions.ask` prompts the user. The `block-credential-exfiltration` hook still fires the Authorization-header detection project-wide, so credential-payload coverage isn't lost.
...
- **registry**: `github-api-host` entry in `detection-registry.json` — only consumer was the auto-mode hook's capability-bucket fallback, which is dead now that `Bash(curl:*)` and `Bash(wget:*)` in `permissions.ask` block all curl/wget under auto-mode. Capability bucket is intentionally empty until a real non-curl entry (e.g. docker exec, terraform show) lands.
...
- **tests**: `run-all.sh` and `run-hook-tests.sh` now guard the failing-suite log dump with an existence check — when a child process is killed mid-run (SIGINT/SIGKILL) before its `.log` file is flushed, the aggregator prints `(no log captured — process likely killed mid-run)` instead of letting `cat` error with `No such file`. Addresses the symptom from `test-runner-reliability` backlog task; root cause of the original mid-run kill remains the open question for that task.
