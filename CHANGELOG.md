...
### Changed
- **skills**: `design-db` SKILL.md § Schema Smith Integration refactored to use the new satellite consumer convention. Removed inlined schema-smith invocation logic and replaced with a reference to the pointer file (`resources/schema-smith-input-spec.md`) for invocation pattern and fallback path.
...
### Notes
- Formalizes the consumer-side pattern established in v2.63.13 (`design-db` runtime fetch). This doc + pointer file pattern becomes the standard for all new skill ↔ satellite integrations.
