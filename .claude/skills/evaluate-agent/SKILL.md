...
6. Update `docs/indexes/evaluations.json` using jq:
   ```bash
   jq --argjson result '<JSON>' '.agents.resources["<name>"] = $result' docs/indexes/evaluations.json > tmp && mv tmp docs/indexes/evaluations.json
   ```
