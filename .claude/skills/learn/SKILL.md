...
allowed-tools: Read, Bash(uv run scripts/lesson_db.py:*), Bash(sqlite3:*)
---
...
**See also:** `/manage-lessons` (lifecycle: promote, archive, crystallize after capture), `scripts/lesson_db.py` (DB layer and CLI), `session-start.sh` hook (nudges for manage-lessons)
...
```bash
uv run scripts/lesson_db.py search "<key phrase from lesson>" --limit 5
```
...
```bash
uv run scripts/lesson_db.py add \
  --text "<lesson text>" \
