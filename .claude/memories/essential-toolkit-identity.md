...
**See also:** `essential-conventions-code_style` for implementation guidelines, `essential-conventions-memory` for memory conventions, `relevant-philosophy-reducing_entropy` for curation philosophy
...
**Curated references** (`.claude/output/reviews/exploration/`) are the lane for "interesting but not actionable now." Don't promote to a real resource until there's a concrete use case.
