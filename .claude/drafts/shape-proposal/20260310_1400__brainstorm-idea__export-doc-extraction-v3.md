...
> **What this is**: An alternative architecture proposal based on reviewing the current MVP (phase 2) and phase 1 deployment. This is not a decided plan - it's a starting point for discussion.
...
## What the Current MVP Does vs What We'd Change

| Area | Current MVP (phase 2) | This proposal | Why |
|------|----------------------|---------------|-----|
| **Preprocessing** | OCR -> embeddings -> vector DB (Qdrant) | Direct text extraction (PDFs assumed text-based) | Simpler. Few document types with known schemas don't need semantic search - just look up by type. |
...
| **Reconciliation** | Present in MVP (details not reviewed yet) | Deterministic Python rules against structured extracted data | Rules should be declarative, testable, auditable. To be compared once MVP reconciliation is reviewed. |
| **Deployment** | Unclear / local | PA Cloud (glue) + AWS (processing) | Reliable, no desktop dependency, team stays in familiar tools |
| **Accuracy tracking** | None visible | Recurring validation against historical data + full audit trail | Non-negotiable for this domain |

## Requirements / Non-Negotiables

These are not optional regardless of which architecture is chosen:
...
6. **Reliable deployment.** Off the Windows desktop machine. No more crashes, no more PA Desktop fragility.
...
- **Volume is low, accuracy is critical** - this favors using the best available model and multi-pass verification over optimizing for speed/cost
